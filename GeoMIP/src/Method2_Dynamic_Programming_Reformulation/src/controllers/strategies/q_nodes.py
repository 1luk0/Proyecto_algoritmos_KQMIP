...
class QNodes(SIA):
    ...
    def aplicar_estrategia(
        ...
        # Filtrar ks válidos según el número de nodos espaciales
        max_nodos = max(self.n, self.m)
        ...
        k_particiones = self.algorithm(vertices, ks_validos)

        resultados = {}
        for k, particion_k in k_particiones.items():
            partes_sistema = []
            for parte in particion_k:
                alcance_parte = [idx for t, idx in parte if t == EFECTO]
            ...
            dist_marginal_k = sistema_k.distribucion_marginal()
            perdida_k = emd_efecto(dist_marginal_k, self.sia_dists_marginales)

            fmt_k = fmt_k_parte_q(particion_k)

            resultados[k] = Solution(
                estrategia=f"{QNODES_LABEL} (k={k})",
                perdida=perdida_k,
                distribucion_subsistema=self.sia_dists_marginales,
                distribucion_particion=dist_marginal_k,
    ...
    def algorithm(self, vertices: list[tuple[int, int]], ks: list[int]):
        """
        ...
        Args:
            ...
            ks (list[int]): Lista de valores k para los que se capturan particiones intermedias.

        Returns:
            dict[int, list[list[tuple[int, int]]]]: Diccionario con las k-particiones capturadas.
        """
        k_particiones = {}
        if len(vertices) in ks:
            k_particiones[len(vertices)] = [[v] if isinstance(v, tuple) else v for v in vertices]
