...
class System:
    ...
    def k_partir(
        ...
        """
        Genera una k-partición del sistema dada una lista de k tuplas (alcance, mecanismo).
        Cada n-cubo es marginalizado según el mecanismo de la partición a la que pertenece
        su índice (según el alcance de esa partición).

        Args:
            particiones: lista de k tuplas (alcance, mecanismo).

        Returns:
            System k-particionado.
        """
        new_sys = System.__new__(System)
        new_sys.estado_inicial = self.estado_inicial
        nuevos_cubos = []
        for cubo in self.ncubos:
            ...
            if mecanismo_correspondiente is not None:
                nuevos_cubos.append(
                    cubo.marginalizar(np.setdiff1d(cubo.dims, mecanismo_correspondiente))
                )
            else:
                nuevos_cubos.append(cubo)
        new_sys.ncubos = tuple(nuevos_cubos)
        return new_sys
