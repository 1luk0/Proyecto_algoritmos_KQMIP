...
class QNodes(SIA):
    ...
    def aplicar_estrategia(
        ...
        # Descartar particiones que fragmenten el tiempo de un mismo nodo
        # El máximo de partes espaciales lógicas es el mayor número de nodos en presente o futuro
        max_nodos = max(self.n, self.m)
        ks_validos = [k for k in ks if k <= max_nodos]
        ...
        for k, particion_k in k_particiones.items():
            ...
            for parte in particion_k:
                ...
                partes_sistema.append((np.array(alcance_parte, dtype=np.int8), np.array(mecanismo_parte, dtype=np.int8)))
            ...
            fmt_k = fmt_k_particion_q(particion_k)
